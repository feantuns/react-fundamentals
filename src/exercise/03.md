...
I learned that ReactDOM escapes all JSX that is passed to it before rendering in
the DOM.

About the exercises already knew the way JSX works, like an HTMLish syntax which
gets converted by Babel or other code compiler into React components using its
default API (.createElement, etc).

JSX is closer to JavaScript than to HTML so the attributes are camelCase, class
becomes className, tabindex becomes tabIndex, etc.

To make a script tag be interpreted by babel you need to set the script `type`
to `text/babel`, so that the browser will ignore it and leave to babel do its
work.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=03%3A%20Using%20JSX&em=felipeantunes542%40gmail.com).
