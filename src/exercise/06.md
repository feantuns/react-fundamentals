...
There are a lot of ways to access the value of an input value on form submit, by
`elements.elementId.value`, and others described in this lecture.

You can also use Refs to keep a reference to the element using the ref prop to
pass a ref created by `React.useRef`, this way you can always access the current
element through ref.current, this way you will get access to the element itself,
so you can use and call any of the input methods or properties like .focus() or
the .value property.

It's a good practice to label an alert message with the `role="alert"` to help
screenreaders.

At most times React create synthetic events which are kind of a copy of native
events to make the apps more performant. If you want to use the native event you
can use `event.nativeEvent`.

`console.dir` gives the DOM properties of a given DOM node.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=felipeantunes542%40gmail.com).
