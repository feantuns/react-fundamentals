...
- I learned that there is a different extension `.mjs` specifically for
  indicating to the browser that that piece of js code is a module.
- Learned that you can export modules with `await` so the module that is
  importing it will wait for the promise to end to be loaded after.
- Learned that you can use setAttribute to set attributes in node elements.
- You can create elements with JS and append them in the document using .append
  or .appendChild
  - There are differences between .append and .appendChild, such as:
  - With append you can append a NodeList, which means more than one element at
    a time
  - With append you can append DOMStrings such as "&lt;p&gt;X&lt;/p&gt;"
  - Append does not return anything, while appendChild returns the appended
    element
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=01%3A%20Basic%20JavaScript-rendered%20Hello%20World&em=felipeantunes542%40gmail.com).
